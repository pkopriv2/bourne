...
func (s memberCore) secret(shard memberShard, login func(KeyPad) error) (Secret, error) {
...
func (m memberCore) newShard(rand io.Reader, secret Secret, login func(KeyPad) error) (memberShard, error) {
	creds, err := extractCreds(login)
	if err != nil {
	...
	enc, err := creds.Encrypt(rand, signer, shard)
