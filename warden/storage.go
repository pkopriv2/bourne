...
type StoredSubscriberAuth struct {
	SignedOracleKey
}
...
func BoltLoadSubscriber(db *bolt.DB, sub string) (s StoredSubscriber, o bool, e error) {
	e = db.View(func(tx *bolt.Tx) error {
		s, o, e = boltLoadSubscriber(tx, sub)
		return e
	})
	return
}

func BoltEnsureSubscriber(db *bolt.DB, sub string) (StoredSubscriber, error) {
	s, o, e := BoltLoadSubscriber(db, sub)
	if e != nil || !o {
...
func BoltStoreSubscriber(db *bolt.DB, sub Subscriber, auth SignedOracleKey) error {
	if err := auth.Verify(sub.Identity.Pub); err != nil {
		return err
	}

	return db.Update(func(tx *bolt.Tx) error {
		if err := boltStoreSubscriber(tx, sub.Identity, sub.Oracle); err != nil {
			return err
		}
...
func BoltLoadSubscriberAuth(db *bolt.DB, sub, method string) (s StoredSubscriberAuth, o bool, e error) {
	e = db.View(func(tx *bolt.Tx) error {
		s, o, e = boltLoadSubscriberAuth(tx, sub, method)
		return e
	})
	return
}

func BoltEnsureSubscriberAuth(db *bolt.DB, sub, method string) (StoredSubscriberAuth, error) {
	s, o, e := BoltLoadSubscriberAuth(db, sub, method)
	if e != nil || !o {
		return s, common.Or(e, errors.Wrapf(StorageInvariantError, "No subscriber auth [%v,%v]", sub, method))
	}
	return s, nil
}

func BoltStoreSubscriberAuth(db *bolt.DB, id, name string, auth SignedOracleKey) error {
	sub, err := BoltEnsureSubscriber(db, id)
	if err != nil {
		return errors.WithStack(err)
	}

	if err := auth.Verify(sub.Identity.Pub); err != nil {
		return errors.Wrapf(err, "Cannot add auth [%v] to subscriber [%v]. Invalid signature.", id, name)
	}

	return db.Update(func(tx *bolt.Tx) error {
		return boltStoreSubscriberAuth(tx, sub.Id(), name, auth)
	})
}

func BoltEnsureDomain(db *bolt.DB, dom string) (StoredDomain, error) {
	d, o, e := BoltLoadDomain(db, dom)
	if e != nil || !o {
		return d, common.Or(e, errors.Wrapf(StorageInvariantError, "Domain not found [%v]", dom))
	}
	return d, nil
}

func BoltLoadDomain(db *bolt.DB, dom string) (s StoredDomain, o bool, e error) {
	e = db.View(func(tx *bolt.Tx) error {
		s, o, e = boltLoadDomain(tx, dom)
		return e
	})
	return
}

func BoltStoreDomain(db *bolt.DB, dom Domain) error {
	issuer, err := BoltEnsureSubscriber(db, dom.cert.Issuer)
	if err != nil {
		return errors.WithStack(err)
	}

	if err := dom.cert.Verify(dom.ident.Pub, issuer.Identity.Pub, issuer.Identity.Pub); err != nil {
		return errors.Wrapf(err, "Unable to create domain [%v].  Invalid certificate.", dom.Id())
	}

	return db.Update(func(tx *bolt.Tx) error {
		if err := boltStoreDomain(tx, dom.Description, dom.ident, dom.oracle); err != nil {
			return err
		}
...
func boltStoreSubscriber(tx *bolt.Tx, identity KeyPair, oracle SignedOracle) error {
	id := identity.Pub.Id()
...
func boltStoreSubscriberAuth(tx *bolt.Tx, sub, method string, k SignedOracleKey) error {
	raw, err := gobBytes(StoredSubscriberAuth{k})
	if err != nil {
...
func boltLoadSubscriberAuth(tx *bolt.Tx, sub, method string) (k StoredSubscriberAuth, o bool, e error) {
	o, e = parseGobBytes(
		tx.Bucket(subscriberAuthBucket).Get(stash.String(sub).ChildString(method)), &k)
	return
}

func boltStoreDomain(tx *bolt.Tx, desc string, identity KeyPair, oracle SignedOracle) error {
	id := identity.Pub.Id()
	...
	raw, err := gobBytes(StoredDomain{desc, identity, oracle})
	if err != nil {
...
func boltLoadDomainAuth(tx *bolt.Tx, dom, sub string) (k OracleKey, o bool, e error) {
