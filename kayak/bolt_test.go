...
func TestBoltLog_CreateSnapshot_Empty(t *testing.T) {
	...
	s, err := createBoltSnapshot(db, NewEventChannel([]Event{}), 0, []byte{})
	assert.Nil(t, err)
...
func TestBoltLog_CreateSnapshot_Config(t *testing.T) {
	...
	s, err := createBoltSnapshot(db, NewEventChannel([]Event{}), 0, expected)
	assert.Nil(t, err)
...
func TestBoltLog_CreateSnapshot_Events(t *testing.T) {
	...
	s, err := createBoltSnapshot(db, NewEventChannel(expected), 2, []byte{})
	assert.Nil(t, err)
...
func TestBoltLog_CreateSnapshot_MultipleWithEvents(t *testing.T) {
	...
	snapshot1, err := createBoltSnapshot(db, NewEventChannel(expected1), len(expected1), []byte{})
	assert.Nil(t, err)

	snapshot2, err := createBoltSnapshot(db, NewEventChannel(expected2), len(expected2), []byte{})
	assert.Nil(t, err)
...
func TestBoltLog_DeleteSnapshot(t *testing.T) {
	...
	db := OpenTestLogStash(ctx)
	snapshot, err := createBoltSnapshot(db, NewEventChannel(events), len(events), []byte{})
	assert.Nil(t, err)
